...
// Inizializzazione standard (Corretta per evitare errori TS7034)
...
function NewEntryForm({
  ...
  const handleSubmit = async (e) => {
    ...
    const logData = {
      // FIX 1: Aggiunto ID temporaneo per i log offline
      id: `temp_${Date.now()}`,
    ...
    // LOGICA ONLINE
    try {
      // Rimuoviamo l'ID temporaneo per non salvarlo su Firestore
...
function HistoryView({
  ...
  const isEditableFree = (log) => {
    // FIX: Se il log è offline (createdAt è numero) o recente, è modificabile
    if (!log.createdAt) return true;
    // Se è un numero (offline), lo trattiamo come timestamp
    const created =
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    {/* Indicatore log offline */}
                    {log.id.startsWith("temp_") && (
                      <span className="text-[9px] font-black text-orange-500 uppercase bg-orange-50 px-2 py-1 rounded-lg border border-orange-200">
                        In attesa di sync
                      </span>
                    )}
...
export default function App() {
  ...
  const [authError, setAuthError] = useState(null); // FIX 3: Stato per errori auth
  const [activeTab, setActiveTab] = useState("dashboard");
  const [logs, setLogs] = useState([]);
  const [localLogs, setLocalLogs] = useState([]); // FIX 2: Stato per i log offline
  ...
  // FIX 2: Funzione per leggere i log offline
  const refreshLocalLogs = () => {
    try {
      const pending = JSON.parse(
        localStorage.getItem("mora_pending_logs") || "[]"
      );
      setLocalLogs(pending.filter((l) => l.type === "log"));
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (pending.length > 0) {
      ...
      refreshLocalLogs(); // Load local logs on start
      if (navigator.onLine) {
        syncPendingData();
      }
    }
  ...
  const syncPendingData = async () => {
    ...
    for (const log of pendingLogs) {
      try {
        // Prepara i dati, rimuove il timestamp fittizio offline e l'id temporaneo
        const { type, createdAt, id, ...logData } = log;
    ...
    if (newPending.length > 0) {
      ...
      refreshLocalLogs(); // Clear local logs from view
      alert(
      ...
      if (currentUser) {
        ...
        const doLogin = async () => {
          try {
            ...
            setAuthError(e.message); // FIX 3: Mostra errore se login fallisce
          }
        };
        doLogin();
      }
  ...
  useEffect(() => {
    ...
    const unsubLogs = onSnapshot(
      ...
      (snapshot) => {
        ...
        // Ordinamento spostato nel render per unire i log locali
        setLogs(fetched);
        setLoading(false);
      }
    );
  ...
  // FIX 1: Merge e Sort sicuro
  const allLogs = useMemo(() => {
    const combined = [...localLogs, ...logs];
    return combined.sort((a, b) => {
      // Gestisce sia Timestamp Firebase ({seconds: ...}) che timestamp numerici (Date.now())
      ...
      >
        {!user ? (
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            {activeTab === "history" && (
              <HistoryView
                logs={allLogs} // FIX 1: Usa allLogs che contiene sia online che offline
                machineMap={machineMap}
                loading={loading}
                isMobile={currentViewIsMobile}
